...
1. Select the MOST APPROPRIATE diagram type from the following options, prioritizing types that leverage SVG's customization capabilities:
   - flowchart (for process flows with custom shapes or animations)
   - hierarchy (for organizational or tree structures with styled nodes)
...
SVG DIAGRAM REQUIREMENTS:
1. Select the MOST APPROPRIATE diagram type from the following options:
   - flowchart (for process flows)
   - hierarchy (for organizational structures)
   - relationship (for network connections)
...
svg_addition_prompt = PromptTemplate(
    input_variables=["section", "slide", "svg_code", "additional_data", "format_instructions"],
