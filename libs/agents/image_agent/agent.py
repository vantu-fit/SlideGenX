...
from libs.models.visualization_models import ListImageSpec
...
class ImageAgent(BaseAgent):
    ...
    def __init__(self, session: Session, searxng_instance: str = gcfg.GlobalConfig.SEARXNG_ENDPOINT, **kwargs):
        """
    ...
    def run(self, section_index : int, slide_index: int, image_path : str, additional_data : str | None = None,
            search_images: bool = True, num_images: int = 1) -> AgentResponse:
        logger.info(f"INPUT IMAGE AGENT: Generate image section: {section_index} - slide #{slide_index} - additional_data: {additional_data}")
        """
        ...
        """
        try:
            # Generate image specifications using the original logic
            ...
            formatted_prompt = self._get_prompt(section=section, slide=slide, additional_data=additional_data)
            self.session.save_prompt(key=section.section_index, type="image", prompt=formatted_prompt)
            ...
            for image_spec in image_specs.images:
                # If search is requested, perform image search
                if search_images:
            ...
            return AgentResponse(
                ...
                message=f"Successfully generated image specifications{' and found matching images' if search_images else ''}",
                data={
            ...
            # Log error
            logger.error(f"Error in Enhanced Image Agent: {str(e)}")
            
            # Return error response
            return AgentResponse(
                status=AgentStatus.ERROR,
                message=f"Error in Enhanced Image Agent: {str(e)}",
                data={}
            )
    
    def _search_and_save_images(self, search_query: str, num_images: int, context_name: str, image_path : str, retry_attempts: int = 2) -> List[Dict[str, Any]]:
        """
        ...
        # Search for images
        self.session.output_message.add(f"Searching for images using query: '{search_query}'", level="info")
        search_results = None
        for attempt in range(retry_attempts):
            search_results = search_tool._run(query=search_query, num_results=num_images)
            if search_results and isinstance(search_results, list) and len(search_results) > 0:
                break
            logger.warning(f"Attempt {attempt + 1}: No images found for query: '{search_query}'. Retrying...")
        ...
        # Log results
        logger.info(f"Successfully processed {len(processed_results)} images out of {len(search_results)} results")
        self.session.output_message.add(f"Retrieved {len(processed_results)} images", level="info")
    ...
    def _download_and_save_image(self, img_url: str, context_name: str, idx: int, save_path : str) -> tuple[str, Optional[str]]:
        """
        ...
        try:
            ...
            logger.info(f"Successfully downloaded and saved image with key: {image_key}")
            ...
            return image_key if 'image_key' in locals() else f"failed_{uuid.uuid4().hex[:8]}", None
